...
11. ### ¿Diferencia entre estado y props?

    | Característica | Estado                          | Props                          |
    |----------------|---------------------------------|--------------------------------|
    | **Origen**     | Gestionado por el componente    | Pasado por el componente padre |
    | **Mutabilidad**| Mutable (con `setState`/hooks)  | Inmutable (solo lectura)       |
    | **Uso**        | Datos internos del componente   | Configuración desde el padre   |

    **Analogía:**  
    - **Props** = Argumentos de una función  
    - **Estado** = Variables declaradas dentro de la función

    **[⬆ Volver al inicio](#table-of-contents)**

12. ### ¿Diferencia en manejo de eventos HTML vs React?

    1. **Nomenclatura:**  
       HTML: `onclick` (minúsculas)  
       React: `onClick` (camelCase)

    2. **Prevenir comportamiento:**  
       HTML: `return false`  
       React: `e.preventDefault()`

    3. **Función:**  
       HTML: Necesita `()` (ej: `activar()`)
       React: Sin `()` (ej: `onClick={activar}`)

    **[⬆ Volver al inicio](#table-of-contents)**

13. ### ¿Qué son los Synthetic Events?

    Son envoltorios cross-browser para eventos nativos del navegador. Unifican el comportamiento entre navegadores.

    **Acceso al evento nativo:**
    ```js
    function handleClick(e) {
      console.log(e.nativeEvent); // Evento nativo del navegador
      e.preventDefault(); // Método compatible
    }
    ```

    **[⬆ Volver al inicio](#table-of-contents)**

14. ### ¿Expresiones condicionales en línea?
    ...
    **Opción 2: `&&` para condiciones simples**
    ```jsx
    {messages.length > 0 && <span>Tienes mensajes</span>}
    ```

    **[⬆ Volver al inicio](#table-of-contents)**

15. ### ¿Para qué sirve la prop `key`?

    **Propósito:** Identificar elementos en listas para optimizar el renderizado.

    **Buena práctica:**
    ```jsx
    {items.map(item => (
      <li key={item.id}>{item.name}</li> // Usar ID único
    ))}
    ```

    **Mala práctica (último recurso):**
    ```jsx
    {items.map((item, index) => (
      <li key={index}>{item.name}</li> // Evitar si el orden puede cambiar
    )}
    ```

    **Reglas:**
    - Las keys deben ser únicas entre hermanos
    - No generarlas dinámicamente (ej: `key={Math.random()}`)

    **[⬆ Volver al inicio](#table-of-contents)**

16. ### ¿Qué es el Virtual DOM?

    Representación ligera en memoria del DOM real. React compara el Virtual DOM con una copia previa para determinar los cambios mínimos necesarios (proceso llamado **reconciliación**).

    **Beneficios:**
    - Mayor rendimiento al minimizar operaciones costosas en el DOM real
...
17. ### ¿Cómo funciona el Virtual DOM?
    ...
    1. **Renderizado inicial:** Crea representación Virtual DOM
    2. **Cambios de estado:** Genera nueva representación Virtual DOM
    3. **Diffing:** Compara con la versión anterior
    4. **Reconciliación:** Actualiza solo los nodos modificados en el DOM real

    **[⬆ Volver al inicio](#table-of-contents)**

18. ### ¿Diferencia entre Shadow DOM y Virtual DOM?

    | Virtual DOM                     | Shadow DOM                     |
    |---------------------------------|--------------------------------|
    | Implementado por React          | Tecnología de navegadores      |
    | Para optimizar actualizaciones  | Para encapsular estilos/scope  |

    **[⬆ Volver al inicio](#table-of-contents)**

19. ### ¿Qué es React Fiber?

    Nueva arquitectura de reconciliación introducida en React 16. Permite:
    - Renderizado incremental (dividir trabajo en chunks)
    - Priorización de tareas
    - Mejor manejo de errores

    **[⬆ Volver al inicio](#table-of-contents)**

20. ### ¿Objetivo principal de React Fiber?
