...
Finally, run `yarn test` or `npm test`:
...
_Flux_ is an application design paradigm used as a replacement for the more traditional MVC pattern. It complements React and the concept of Unidirectional Data Flow. Facebook uses this pattern internally with React.

---

103. ### What is Redux?

_Redux_ is a predictable state container for JavaScript apps based on the Flux design pattern. It can be used with React or any other library. It's tiny (~2kB) and has no dependencies.

---

104. ### What are the core principles of Redux?

1. **Single source of truth:** The whole state lives in a single store.
2. **State is read-only:** State changes via actions only.
3. **Changes via pure functions:** State changes are done by reducers.

---

105. ### What are the downsides of Redux compared to Flux?

- Avoiding mutations is required.
- You have to choose packages carefully.
- Flow integration is not as advanced.

---

106. ### What is the difference between `mapStateToProps()` and `mapDispatchToProps()`?

`mapStateToProps` lets your component access state.
...
`mapDispatchToProps` lets your component dispatch actions.
...
No. Reducers must be pure functions. Dispatching inside reducers is an anti-pattern.

---

108. ### How to access Redux store outside a component?

Export the store created with `createStore()`:
...
- Circular dependencies complicate the model.
- High frequency of data changes is hard to track.
- Undo logic is complex.
